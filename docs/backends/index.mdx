...
PocketPaw supports **eight agent backends** built on a unified `AgentBackend` protocol. Each backend provides different capabilities and trade-offs. All backends yield standardized `AgentEvent` objects, so switching is seamless.

## Backend Comparison

| Feature | Claude SDK | OpenAI Agents | Google ADK | Deep Agents | Pydantic AI | Codex CLI | OpenCode | Copilot SDK |
|---------|-----------|---------------|------------|-------------|-------------|-----------|----------|-------------|
| Status | Production | Beta | Beta | Beta | Beta | Beta | Beta | Beta |
| Streaming | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes |
| Tools | Yes | Yes | Yes | Yes | Dispatch-only | Yes | Yes | Yes |
| MCP | Yes | No | Yes | .mcp.json | Opt-in | Yes | No | No |
| Multi-turn | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes |
| Local models | Ollama | Ollama | No | Ollama | Ollama | No | No | No |
| OpenRouter | Partial | Yes | No | Yes | Yes | No | No | No |
| LiteLLM | Yes | Yes | Yes | Yes | Default | No | No | Yes |
| Runs in-process | No | Yes | Yes | Yes | Yes | No | No | Yes |
| Required key | [Anthropic API key](https://console.anthropic.com/settings/keys) | OpenAI | Google | None | None | OpenAI | None | None |

<Callout type="info">
Serving many simultaneous sessions? The **Runs in-process** row is the one that
matters. `claude_agent_sdk` spawns a subprocess per concurrent run at roughly
300–500 MB each, which is fine locally and costly on a shared server.
[Pydantic AI](/backends/pydantic-ai) was added for that case.
</Callout>
...
Most backends have access to PocketPaw's 50+ built-in tools (file operations, web search, memory, integrations, and more). Tools are wrapped differently for each backend's expectations through a unified tool bridge:
...
- **Pydantic AI** wraps tools as `Tool` objects whose schema is derived from a synthesized signature
- **Codex CLI** and **OpenCode** inject tool instructions into the system prompt with a CLI bridge

**Pydantic AI is the exception**: it is dispatch-only and ships no local shell or
filesystem tools, because an in-process backend cannot give each tenant its own
jail. See [Pydantic AI](/backends/pydantic-ai#dispatch-only).
...
<CardGroup>
  ...
  <Card title="Deep Agents" icon="lucide:workflow" href="/backends/deep-agents">
    LangChain/LangGraph backend with built-in planning, subagent delegation, and multi-provider support.
  </Card>
  <Card title="Pydantic AI" icon="lucide:gauge" href="/backends/pydantic-ai">
    In-process backend built for concurrency. Reaches models through the LiteLLM proxy; dispatch-only by design.
  </Card>
