...
<Callout type="info">
  The container binds to `0.0.0.0:8888` by default (set via `POCKETCLAW_WEB_HOST` and `POCKETCLAW_WEB_PORT` environment variables in the Dockerfile).
</Callout>
...
```bash
POCKETCLAW_OLLAMA_HOST=http://ollama:11434
```
...
The `.env.example` file is organized by section (LLM, channels, memory, tools, security) and documents every available `POCKETCLAW_` variable. See the [Configuration reference](/getting-started/configuration) for full details.
...
A named Docker volume (`pocketpaw-data`) is mounted at `/home/pocketpaw/.pocketclaw` to persist:
