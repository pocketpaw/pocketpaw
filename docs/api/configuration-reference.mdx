...
| Setting | Env Variable | Type | Default | Description |
|---------|-------------|------|---------|-------------|
| `refero_api_token` | `POCKETPAW_REFERO_API_TOKEN` | string | — | Refero API token. Enables the design-research tools that ground generated UI in real shipped design systems: `refero_design_styles` / `refero_design_screens` (BaseTools, for the pydantic_ai and other non-SDK backends) and the `pocketpaw_refero` in-process MCP server exposing `search_styles` / `get_style` / `search_screens` (for the claude_agent_sdk backend, which cannot see a BaseTool). Requires a paid Refero plan. **Unset degrades to empty results everywhere** — a site build proceeds without design research rather than failing, exactly like an unconfigured stock-photo provider. Granted to `/sites` on every mode; other restrictive surfaces see it only if they name the ids. **Licensing:** a per-seat Refero plan is licensed to one user and forbids sharing access, so a token serving a multi-tenant deploy needs Refero's usage-based integration agreement, not a Pro seat. |
| `refero_endpoint` | `POCKETPAW_REFERO_ENDPOINT` | string | `""` | Override the Refero MCP endpoint. Empty uses the documented default (`https://api.refero.design/mcp`). For pointing at a proxy or staging host without a code change; no reason to set it otherwise. |
...
| `sites_mcp_servers` | `POCKETPAW_SITES_MCP_SERVERS` | string | `""` | Comma-separated names of EXTERNAL MCP servers (from `~/.pocketpaw/mcp_servers.json`) that the **/sites** surface may call — e.g. `refero` for live design research. Empty grants none, so an install that has not opted in is unaffected. `/sites` runs a hard allow-list of fully-qualified `mcp__<server>__<tool>` ids, but an external server's tool names are unknown until the client connects, so it is allow-listed wholesale with a bare `mcp__<server>` token; naming a server here puts that token in the /sites allow set. This scopes the grant to **that surface alone** — unlike `ALWAYS_ALLOWED_MCP_SERVERS`, which hands a server to every surface. A name matching no configured server is inert, not an error. Read once at startup. |

| `inspo_mcp_url` | `POCKETPAW_INSPO_MCP_URL` | string | `""` | Override the endpoint the bundled design-research tools call — the `pocketpaw_inspo` server and the `inspo_*` BaseTools. Empty uses the hosted archive at `https://inspomcp.dev/api/mcp`. Exists because the hosted service rate-limits **per IP** and a multi-tenant deploy presents one egress IP for all its tenants, so a busy install can exhaust the limit for everyone at once; the upstream is MIT with a documented self-host path. Read per call, so it takes effect without a restart. |

### Design research on /sites

**Nothing to configure.** `pocketpaw_inspo` is a bundled in-process MCP server,
like `pocketpaw_stock` and `pocketpaw_icons` — it ships with the deploy and the
/sites create preamble names its tools unconditionally.

It wraps an archive of ~2,300 captured pages across ~830 real shipped sites,
each carrying a DESIGN.md extracted from the live DOM (role-tagged palette, type
ramp, spacing scale, macrostructure), and exposes two tools:
`research_page_design` (a brief in, a macrostructure and real exemplars out) and
`get_reference_design_system` (one exemplar's DESIGN.md).

The same archive reaches the pydantic_ai and other non-SDK backends through two
BaseTools, `inspo_research_page_design` and `inspo_reference_design_system`,
which cannot see an in-process MCP server. Both surfaces share one code path
(`pocketpaw.tools.builtin.inspo`), so `inspo_mcp_url` applies to both.

The preamble names tools by their SDK ids (`mcp__pocketpaw_inspo__…`). The
pydantic_ai backend bridges the same in-process servers as `<server>_<tool>`,
so it rewrites those ids in the prompt to its own names
(`pocketpaw_inspo_research_page_design`) before the turn runs. One preamble
serves every backend.

The /sites **create** preamble uses them in `PHASE 1b`, after the agent commits
to a direction and before it locks tokens — once, for composition: which
sections, in what order, what carries the fold. It does not relax the embedded
design system, which still outranks any reference on a visual value, and it does
not relax the rotation rule that stops two similar briefs resolving to the same
page.

<Note>
This was briefly shipped as an **external** server behind
`POCKETPAW_SITES_MCP_SERVERS` (pocketpaw#2204). That took two switches — install
the server, then grant it — both defaulting off and neither implying the other,
so a deploy of the code alone changed nothing and said nothing about why. The
server is bundled now and that setting is no longer involved in design research.
It remains available for granting *other* external servers to `/sites`.
</Note>
