...
The one grant this is written for today is `inspo` — a free, unauthenticated
archive of ~2,300 captured pages across ~830 real shipped sites, each carrying a
DESIGN.md extracted from the live DOM (role-tagged palette, type ramp, spacing
scale, macrostructure). Turning it on is two steps, deliberately:

1. **Register the server.** Dashboard → MCP → install the **Inspo** preset. It
   asks for no credentials, because the server takes none.
2. **Grant it to /sites.** Set `POCKETPAW_SITES_MCP_SERVERS=inspo` and restart.
   The setting is read once per process.

Step 1 alone registers the server without letting the site agent act on it;
step 2 alone is inert, because the name matches nothing. Both are needed, and
they are separate because they are separate decisions: an external MCP server,
once registered, is reachable from any surface that does not pin its own
allow-list, while the grant is what scopes it to `/sites`.

With both in place the /sites **create** preamble grows a `PHASE 1b` step that
tells the agent to ground its direction in real pages — once, for composition
(which sections, in what order, what carries the fold) — before it locks tokens.
It does not relax the embedded design system, which still outranks any reference
on a visual value, and it does not relax the rotation rule that stops two
similar briefs resolving to the same page. With either step missing the preamble
is byte-identical to an install that never heard of the feature.
