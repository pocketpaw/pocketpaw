...
| `sites_billing_enforced` | `POCKETPAW_SITES_BILLING_ENFORCED` | bool | `false` | Per-SITE billing enforcement, independent of `billing_enforced`. When `true`, the Paw Sites seams enforce: the custom-domain capability gate, the custom-domain count cap (how many sites in a workspace may carry one, and how many hostnames a free site may carry) and the visitor concierge entitlement. Set **either** this or `billing_enforced` and those seams are live. It exists because the two decisions are unrelated — charging for custom domains should not also start rejecting chat runs with 402. Explicitly outside its scope and still governed by `billing_enforced` alone: chat-run credit blocks, the seat cap, the pocket cap, the connector cap, the daily call budget and the storage cap. With both off the seams do no extra database read, so OSS / self-host is unaffected. **Does not gate the visitor concierge** — that moved to its own flag on 2026-08-21 after enabling this one removed the concierge from every site. |
| `sites_concierge_enforced` | `POCKETPAW_SITES_CONCIERGE_ENFORCED` | bool | `false` | Visitor-concierge entitlement enforcement, gated by neither `billing_enforced` nor `sites_billing_enforced`. When `true`, a site serves its concierge only if its plan tier sells one **and** its per-site subscription is active; the owner's own `concierge_enabled` switch is checked first regardless and always wins. **Leave this off until a tier can actually sell a concierge.** No tier below the unbuilt `staff` sells one, and `POCKETPAW_DODO_SITE_PRODUCTS` is configured in no deployment, so no site can reach an active per-site subscription — turning it on today removes the concierge everywhere with no way for a customer to buy it back. |
