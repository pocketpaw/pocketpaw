...
**The gate covers the interactive station only.** It has a single call site, the
`belt_propose_change` tool. The headless develop runner writes its diff straight
onto the queued Action, so it runs no checks and records no verdict regardless of
these settings. The per-diff human gate still applies there.
