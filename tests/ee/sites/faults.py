...
def ok_sentinel(**over: Any) -> dict[str, Any]:
    ...
    base = {
        ...
        # ``verify_artifact`` now compares the two: a sentinel promising 4096 bytes beside a
        # 169-byte artifact is a TRUNCATED transfer, so a hardcoded number here would make
        # every happy-path test in the ladder fail for the wrong reason.
...
def artifact_with_node_modules() -> bytes:
    ...
    The nested shape the include-list does NOT exclude (``dist/node_modules/`` becomes
    ``./node_modules/`` once packed with ``-C dist``), which is what makes the byte gate
    worth having on top of the construction. Deliberately a VALID tar: rejecting garbage is
    the easy half, and the interesting half is rejecting something that opens cleanly.
    """
    return tar_bytes(
        {
