...
- **Claude Code skills** (`claude_skills/`) — Bundled Claude Code SDK skill files (markdown with YAML frontmatter) that auto-install to `~/.claude/skills/<name>/` on dashboard boot. The chat agent invokes them on demand instead of carrying the heavy guidance in its always-on system prompt. Distinct from `pocketpaw/skills/` (AgentSkills / skills.sh ecosystem). Currently bundles `pocketpaw-create-pocket` — the pattern-first pocket-creation workflow with the 150-widget catalog reference. Opt out via `POCKETPAW_AUTO_INSTALL_CLAUDE_SKILLS=false`. See `docs/internal/2026-05-claude-skills.md` for the full design + how to add a new skill.
