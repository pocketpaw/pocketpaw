...
- **Bundled skills** (`bundled_skills/`) — AgentSkills-format SKILL.md files PocketPaw auto-installs to `~/.claude/skills/<name>/` on dashboard boot. That path is one of the three locations PocketPaw's own `SkillLoader` scans (`pocketpaw.skills.SKILL_PATHS`), so bundled skills work across **every** chat backend via the `/skill-name` slash command. claude_agent_sdk additionally auto-discovers them for natural-language invocation. Distinct from `pocketpaw/skills/` (the runtime loader/executor) — this module is the *shipping* side. Currently bundles `pocketpaw-create-pocket` (pattern-first workflow with 150-widget catalog reference). Opt out via `POCKETPAW_AUTO_INSTALL_BUNDLED_SKILLS=false`. See `docs/internal/2026-05-bundled-skills.md` for design + how to add a new skill.
