...
from ``ee.cloud.chat.agent_service.current_user_id`` so multi-tenancy
...
def build_tools_for_backend(
    ...
    *,
    ...
    Resolves the active user from ``ee.cloud.chat.agent_service``
...
def _build_initiate_connection_tool(backend_kind: str, composio_client: Any) -> Any | None:
    ...
    """
    from ee.cloud.composio.connection_tool import initiate_connection_sync
    ...
    if backend_kind == BACKEND_CLAUDE_SDK:
        ...
        async def initiate_connection(args: dict[str, Any]) -> dict[str, Any]:
            ...
            from ee.cloud.composio import service as svc
...
def _build_verify_connection_tool(backend_kind: str, composio_client: Any) -> Any | None:
    ...
    """
    from ee.cloud.composio.identity import probe_identity_sync
...
def _provider_class_for(backend_kind: str) -> tuple[Any, Any]:
    ...
    own backend, not the rest of ``ee.cloud.composio``.
    """
