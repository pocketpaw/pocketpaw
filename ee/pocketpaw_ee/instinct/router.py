...
def _assert_gated_workspaces(action: Any, current_workspace: str) -> None:
    ...
    # paw-bar ``_customer_reply`` (B0 H2). Added when the growth branch's
    # dispatcher met it in a merge: the registry predated this kind, and
    # adopting the dispatcher without it would have silently dropped the
    # gate — a ws-A admin could approve a ws-B decision and the delivery
    # hook, which routes by the BLOB's workspace, would flip a row in ws-B.
