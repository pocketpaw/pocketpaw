...
def _create_preamble(meta: SurfaceMeta) -> str:
    ...
    * ``"ripple"`` — a ripple widget landing spec via the pocket specialist →
    ...
    return (
        ...
        "1. AUTHOR THE TOKENS. Do NOT reach for a canned design-system library — "
        "the embedded pocketpaw-design-taste system IS your design authority. From "
        "your chosen visual identity + aesthetic family (Module 2), author the "
        "token set yourself (`--ink`, `--bg`, `--accent`, `--radius`, `--shadow`, "
        "the display + body font faces): a mandatory background architecture "
        "(never a plain #fff/#000 page), ONE accent under ~80% saturation with the "
        "highest contrast reserved for the primary CTA, off-black/off-white (never "
        "#000/#fff), a bold typographic pairing (not Inter for premium), diverse "
        "section compositions (the default AI sequence and two consecutive "
        "same-layout sections are banned), Tier-0 CSS-only motion, and ZERO "
        "em-dashes. Do NOT default to a warm / earthy palette just because the "
        "business is a cafe, salon, or shop — rotate the identity and reseed the "
        "accent so two similar briefs never look alike.\n"
        "2. CUSTOM COLORS. If the user gave a brand color, call "
        "`mcp__pocketpaw_palette__scale_from_color` with the hex to get a proper "
        "tonal scale and key your `--accent` off it. If they gave a logo or "
        "reference-image URL, call `mcp__pocketpaw_palette__extract_palette` on it "
        "and key the palette off that.\n"
