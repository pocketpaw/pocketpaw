...
class SurfaceMeta:
    ...
    # Code surface hints. ``project_name`` names the open project.
    # ``current_dir`` is the absolute folder of a LOCAL-folder project (desktop
    # app only); the code handler reads it via ``local_code_root``. The other
    # three are legacy and read by nothing.
