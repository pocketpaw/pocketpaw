...
# The SERVER-WORKER counter (SA-3), also at the project root. A SECOND filename rather
# than one name carrying different contents, because the two are not interchangeable:
# the shim imports the adapter's ``_worker.js`` and cannot load in a project that has
# none, and the entry serves through ``ASSETS`` and would drop a dynamic site's server
# rendering entirely. A publish writes exactly one of them and deletes the other, so a
# wrong file under a shared name would deploy without anything noticing.
SHIM_FILENAME = "_paw_analytics_shim.js"
...
def build_entry_js(*, site_id: str, secret: str) -> str:
    ...
    SA-3 — everything above the default handler moved to :func:`_counting_core`, which
    ``build_shim_js`` embeds too. What is left here is the ONE thing the two generated
    files disagree about: where the response comes from. This one asks the ``ASSETS``
    binding, because on the assets-only branch there is nothing else to ask.
    """
...
{_counting_core(site_id=site_id, secret=secret)}
export default {{
...
def _counting_core(*, site_id: str, secret: str) -> str:
    """The COUNTING HALF of a generated entry, shared by both of them.

    Constants, the bot filter, the device class, the visitor hash and ``count`` — every
    line between the file header and the default handler. ONE copy, because the two
    entries this module generates (the assets-only counter and the server-worker shim)
    differ only in where the response comes from, and a second transcription of the
    hash, the bot net or the failure-soft wrapper is how the two stop agreeing about
    what a row means.

    Not public: ``build_entry_js`` and ``build_shim_js`` are the seam, and a caller
    holding half a module has nothing it can deploy.
    """
    return f"""const SITE_ID = {json.dumps(site_id)};
...
def worker_specifier(output_rel: str) -> str:
    """The import specifier the shim reaches the adapter's worker by.

    The shim sits at the PROJECT ROOT and the worker sits inside the build output, so
    the specifier is that output dir plus ``_worker.js``, relative. Normalised to posix
    and forced to start with ``./``, because a bare ``build/_worker.js`` is a BARE
    SPECIFIER to a bundler: it would be resolved as a package rather than as a path, and
    the build would fail on a module sitting right there.
    """
    rel = posixpath.normpath(posixpath.join(output_rel.replace("\\", "/"), "_worker.js"))
    return rel if rel.startswith(("./", "../")) else f"./{rel}"


def build_shim_js(*, site_id: str, secret: str, output_rel: str) -> str:
    """The Worker entry for the SERVER-WORKER branch: count, then delegate.

    ripple and dynamic svelte already have a ``main`` — the worker adapter-cloudflare
    emitted, which is the renderer. A counter cannot be put in front of that from a
    config key, so this generates a module that IMPORTS it, wraps its ``fetch``, and
    becomes ``main`` in its place. The adapter's ``_worker.js`` is never touched: the
    adapter owns that file and rewrites it on every build, so an edit would survive
    ...
    ``...worker`` is not decoration: it forwards any OTHER handler the adapter grows
    (``scheduled``, ``queue``, ``email``) instead of silently dropping it, which an
    object literal naming only ``fetch`` would do.

    THE DELEGATION IS DELIBERATELY NOT WRAPPED IN A TRY. A throw from the site's own
    worker is the site's failure and has to surface exactly as it would without this
    shim in front of it; swallowing it would replace a clear error with a blank
    response nobody can trace. Failure-soft governs the COUNTING, which is worth
    nothing next to the page. It does not mean hiding the page's own errors.

    ``output_rel`` is the build output dir relative to the project, resolved off the
    ARTIFACT by ``engines.resolve_static_output_rel`` rather than guessed from the
    engine name.
    """
    return f"""// Generated by ee/pocketpaw_ee/sites/analytics_worker.py at publish. Do not edit:
// a republish overwrites this file.
//
// Counts one Analytics Engine row per delivered page, then returns the response the
// site's OWN worker produced, untouched. That worker is adapter-cloudflare's, it is
// imported below, and nothing here modifies it.
//
// Every counting path is failure-soft — a throw here must never cost the visitor the
// page. The delegation is not: the site's own failures stay the site's to report.

import worker from "{worker_specifier(output_rel)}";
...
export default {{
  ...
  async fetch(request, env, ctx) {{
    ...
    try {{
      // The same rule the assets-only entry applies, for the same reason: only a
      // delivered page is a pageview. A SvelteKit worker answers redirects and 404s
      // too, and the asset router's fallback sends every unmatched path here.
      if (response && (response.status === 200 || response.status === 304)) {{
        const pending = count(request, env);
        if (ctx && typeof ctx.waitUntil === "function") ctx.waitUntil(pending);
      }}
    }} catch (err) {{
      // Counting must never reach the response. See the note in analytics_worker.py.
    }}
    return response;
  }},
}};
