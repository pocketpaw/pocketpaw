...
**Send timeout (2026-08-18).** Each `send_json` is wrapped in `asyncio.wait_for(..., timeout=SEND_TIMEOUT_SECONDS)` (5s). uvicorn's `websockets` protocol awaits `drain()` inside `send()` with a 64 KiB high-water mark, so a client that stopped reading back-pressures the write — and because `InProcessBus.publish` awaits `send_to_user` per audience member inline in the emitting request, one stuck socket used to stall delivery to everyone else. A timed-out send is treated as a dead socket: it is `disconnect()`ed, its transport is closed best-effort (it is still open, unlike a raised send, and the receive loop would otherwise keep ponging the client), not counted as delivered, and never retried (cancelling a `websockets` send mid-drain leaves the connection unusable). The frontend bus reconnects and refetches on `system.reconnected`.
