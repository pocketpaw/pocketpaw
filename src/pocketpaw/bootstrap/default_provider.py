...
async def get_context(self) -> BootstrapContext:
    """Load context from files and wrap identity in reinforcement tags."""
    identity = (self.base_path / "IDENTITY.md").read_text().strip()
    soul = (self.base_path / "SOUL.md").read_text().strip()
    style = (self.base_path / "STYLE.md").read_text().strip()

    # Reinforce core identity with XML tags
    reinforced_identity = (
        f"<identity_context>\n"
        f"{identity}\n"
        f"{soul}\n"
