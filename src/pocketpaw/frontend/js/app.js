...
function app() {
  ...
  return {
    ...
    init() {
      ...
      if (masterToken) {
        ...
        fetch("/api/auth/session", {
          ...
          .catch(() => {
            this.log(
              "Auth token captured (session exchange unavailable)",
              "info",
            );
          });
      }
      ...
      if (!hasToken && !masterToken) {
        originalFetch("/api/channels/status")
          .then((resp) => {
            if (resp.ok) {
    ...
    setupSocketHandlers() {
      ...
      socket.on("intention_created", (data) =>
        this.handleIntentionCreated(data),
      );
      socket.on("intention_updated", (data) =>
        this.handleIntentionUpdated(data),
      );
      socket.on("intention_toggled", (data) =>
        this.handleIntentionToggled(data),
      );
      socket.on("intention_deleted", (data) =>
        this.handleIntentionDeleted(data),
      );
      ...
      socket.on("skill_received", (data) =>
        console.log("Skill received", data),
      );
    ...
    handleSettings(data) {
      ...
      if (s.agentStatus) {
        this.log(
          `Agent: ${s.agentStatus.backend} (available: ${s.agentStatus.available})`,
          "info",
        );
      ...
      if (
        ...
        !localStorage.getItem("pocketpaw_setup_dismissed")
      ) {
    ...
    getBackendDescription(backend) {
      const descriptions = {
        claude_agent_sdk:
          "Built-in tools: Bash, WebSearch, WebFetch, Read, Write, Edit, Glob, Grep. Works with Anthropic & Ollama.",
        pocketpaw_native:
          "Custom orchestrator with shell, files, memory tools. Works with Anthropic & Ollama.",
        open_interpreter:
          "Experimental — Standalone agent. Works with local LLMs (Ollama) or cloud APIs.",
      };
