# sdk_mcp_meetings.py — in-process MCP server exposing the meetings entity.
# Created: 2026-05-19 — wires ee/cloud/meetings/service into the cloud
#   chat agent so the LLM can schedule, list, cancel, search, and pull
#   transcripts from Zoom + Google Meet natively. Mirrors the
#   ``sdk_mcp_tasks.py`` pattern exactly.
#
...
# set by ``ee.cloud.chat.agent_router._run_agent_stream``. When invoked
...
# ---------------------------------------------------------------------------
# Tool handlers — thin wrappers over ee.cloud.meetings.service
# ---------------------------------------------------------------------------
...
async def _schedule_meeting_handler(args: dict) -> dict:
    ...
    from ee.cloud.meetings import service as ms
    from ee.cloud.meetings.dto import CreateMeetingRequest
...
async def _list_meetings_handler(args: dict) -> dict:
    ...
    from ee.cloud.meetings import service as ms
    from ee.cloud.meetings.dto import ListMeetingsRequest
