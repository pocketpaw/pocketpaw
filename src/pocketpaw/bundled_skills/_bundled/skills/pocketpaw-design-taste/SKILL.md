...
- [ ] **Anti-Slop Cleanliness:** ZERO em-dashes anywhere visible; copy self-audit done (specific believable names/numbers/brand, no filler verbs, specificity over abstract benefits); no duplicate CTA intent; the owner's uploaded images checked via `list_site_assets` and used where they fit, real images via `search_stock_images` with credit, icons via `search_icons`, manifest assets used at their native medium (a video stays a video), no emoji UI, no div-based fake screenshots.
